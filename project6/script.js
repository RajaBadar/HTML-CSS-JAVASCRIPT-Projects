...
// Select a word from list at random
...
function renderWord(){
    //split the random word  into individuals characters as an array, map over the array,
    //for each letter , create a span element and only display the letter if its present 
    // in the correct letters array
    ...
    if (word === randomWord){
        //if the word matches the random word set the game over message
        gameoverMessage.innerText = 'You Won';
        //dispaly the gameover container
...
function displayNotification(){
    //display notification in the window
    notificationElement.classList.add('show');
     //remove the notification after 1 sec
     setTimeout(() => {
        notificationElement.classList.remove('show');
     },1000)
};

//function to update by UI based on incorrect letterguess
...
window.addEventListener('keydown', e => {
    ...
    if (e.keyCode >= 65 && e.keyCode <= 90 ) {
        ...
        if (randomWord.includes(letter)){
            //if the randomword has the letter, check to see if letter is already in correctletter array
...
//listen for a click on the palybtb
